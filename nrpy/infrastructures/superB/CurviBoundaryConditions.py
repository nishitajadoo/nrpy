...
"""
    ...
    params = "const commondata_struct *restrict commondata, const params_struct *restrict params, const bc_struct *restrict bcstruct, const int NUM_GFS, REAL *restrict gfs, const int *gfs_to_sync, const int8_t *gf_parity_types"
    body = r"""
...
#pragma omp parallel for collapse(2)  // spawn threads and distribute across them
  for (int which_gf = 0; which_gf < NUM_GFS; which_gf++) {
    for(int pt=0;pt<bc_info->num_inner_boundary_points;pt++) {
      ...
      gfs[IDX4pt(gfs_to_sync[which_gf], dstpt)] = bcstruct->inner_bc_array[pt].parity[gf_parity_types[gfs_to_sync[which_gf]]] * gfs[IDX4pt(gfs_to_sync[which_gf], srcpt)];
...
###############################
## Modify apply_bcs_outerextrap_and_inner() to apply the bcs to specific gridfunctions instead of all evolved gridfunctions.
##
def register_CFunction_apply_bcs_outerextrap_and_inner_specific_gfs() -> None:
    """Register C function for filling boundary points with extrapolation and prescribed bcstruct for specific gfs."""
...
#pragma omp for  // threads have been spawned; here we distribute across them
          for(int idx2d=0;idx2d<bc_info->num_pure_outer_boundary_points[which_gz][dirn];idx2d++) {
            ...
            for (int which_gf = 0; which_gf < NUM_GFS; which_gf++) {
              ...
              gfs[IDX4pt(gfs_to_sync[which_gf], idx_offset0)] =
                  +3.0 * gfs[IDX4pt(gfs_to_sync[which_gf], idx_offset1)] - 3.0 * gfs[IDX4pt(gfs_to_sync[which_gf], idx_offset2)] + 1.0 * gfs[IDX4pt(gfs_to_sync[which_gf], idx_offset3)];
  ...
  //              STEP 2 OF 2.
  apply_bcs_inner_only_specific_gfs(commondata, params, bcstruct, NUM_GFS, gfs, gfs_to_sync, gf_parity_types);
"""
    cfc.register_CFunction(
        includes=includes,
        desc=desc,
